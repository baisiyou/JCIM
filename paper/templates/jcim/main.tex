...
\begin{itemize}
  ...
  \item \texttt{45\_package\_review\_snapshot.sh} --- build the anonymous review tarball.
\end{itemize}

An \emph{anonymous} reproducibility snapshot is packaged for submission (code, environment pins, frozen manifests, checkpoint, and the scripts above). The cover letter states the anonymous archive URL and SHA-256; that link must be reviewer-accessible without revealing author identity. A public, de-anonymized release accompanies camera-ready. Raw MOSES molecules follow the original MOSES distribution terms~\citep{polykovskiy2020moses}.
